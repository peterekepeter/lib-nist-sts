...
namespace Nist
{
	...
	struct Results {
		...
		struct NonOverlapping { unsigned int templates; unsigned int *W; double *chi2; double *p_value; } nonoverlapping;
		struct Overlapping { unsigned int nu[6]; double chi2, p_value; } overlapping;
		struct Universal { double p_value, sum, phi; } universal;
		struct ApproximateEntropy { double ApEn[2], chi_squared, p_value; unsigned int *P, pp; } approximate_entropy;
